...
st.set_page_config(
    page_title="Ask Questions Deepseek Qwen",
    page_icon="❓",
)
...
def clear_chat_history():
    st.session_state.deepseekqwen_messages = [
...
def run():
    st.title("Ask Questions Deepseek Qwen")
    ...
    if "deepseekqwen_messages" not in st.session_state.keys():
        st.session_state.deepseekqwen_messages = [
            {"role": "assistant", "content": "Ask me questions on the website."}
        ]

    # Display previous chat messages
    for message in st.session_state.deepseekqwen_messages:
        with st.chat_message(message["role"]):
            st.write(message["content"])

    # User input for questions
    if prompt := st.chat_input("Ask a question about the crawled content:"):
        st.session_state.deepseekqwen_messages.append({"role": "user", "content": prompt})
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                ...
                if response:
                    ...
                    st.write(answer)
                    st.session_state.deepseekqwen_messages.append({"role": "assistant", "content": answer})
                else:
